...
/**
 * Instantiates a class by its fully qualified class name (FQCN) and accepts a
 * variable number of arguments. Comparable to call_user_func but for classes
 *
...
/**
 * Instantiates a class by its fully qualified class name (FQCN) and accepts an
 * array of arguments. Comparable to call_user_func_array but for classes
 *
 *     $args = array('John', 'Smith');
 *     $object = class_new_args(__NAMESPACE . '\\Contact', $args);
 *
 * @param string $fqcn The fully qualified class name to instantiate
 * @param array  $args An array of arguments to pass to the class's constructor
 *
 * @return object
 */
...
/**
 * Creates a function from another callable that accepts an array as input and
 * applies the original callback to every item in the array. It's essentially a
 * partially applied array_map function
 *
...
/**
 * Creates a function from another callable that accepts an array as input and
 * applies the original callback to every item in the array as reducer. It's
 * essentially a partially applied array_reduce function
 *
...
function func_reduce($callable, $defaultInitial = null)
{
    ...
    return function (array $input, $userInitial = null) use ($callable, $defaultInitial) {
        return array_reduce($input, $callable, $userInitial ?: $defaultInitial);
    };
}
 ...
 * @param bool|array $preserveKeys Whether or not to preserve the keys of the
 *                                 keys of the array, or, if an array of keys
 *                                 is specified, only those will be preserved
 *
 * @return array
 * @throws InvalidArgumentException if the value can't be coerced into an array
 */
function arrayval($var, $preserveKeys = false)
{
    if (is_array($var)) {
        ...
        throw new \InvalidArgumentException('The provided variable could not be coerced into an array.');
    }
